...
<body>
  ...
  <div class="grid grid-flow-row auto-rows-max md:auto-rows-min m-4">
    ...
    <form action="{{ route('alunos.search') }}" method="POST" class="form-inline">
      ...
      <input type="search" name="filter" placeholder="Procure pelo Nome:" class="form-control" value="{{ $filters['filter'] ?? '' }}">
  ...
  <form action="{{ route('turmas.aluno.edit.bloco') }}" method="POST" class="">
    ...
    <div class="grid grid-flow-row auto-rows-max md:auto-rows-min m-4">
      ...
      @if ($alunosCont > 0)
        <button type='subimit' class='btn btn-outline-info mb-4' id="btEditBloc" disabled
          title="Marque ao menos uma das caixinha para liberar esse botão."><b>Editar em Bloco</b></button>
      @endif
        ...
        <tbody>
          @foreach ($alunos as $aluno)
            <tr>
              ...
              <td> &nbsp;<span><input type='checkbox' name='aluno_selecionado[]' class='checkbox' value='{{ $aluno->uuid }}/{{ $turma->id ?? '' }}'>
